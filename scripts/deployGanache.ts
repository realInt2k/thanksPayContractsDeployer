...
async function deployContract(name: string, args: any[] | null) {
  ...
  if(name === "ThanksSecurity") {
    contractAddresses["ganache"]["THANKS_PAY_SECURITY_ADDR"] = soldier.address;
  } else if(name === "ThanksData") {
    contractAddresses["ganache"]["THANKS_PAY_DATA_ADDR"] = soldier.address;
  } else if(name === "ThanksPayCheck") {
    contractAddresses["ganache"]["THANKS_PAY_CHECK_ADDR"] = soldier.address;
  } else if(name === "thanksPayMain") {
    contractAddresses["ganache"]["THANKS_PAY_MAIN_ADDR"] = soldier.address;
  } else if(name === "thanksPayRelay") {
    contractAddresses["ganache"]["THANKS_PAY_RELAY_ADDR"] = soldier.address;
  }
